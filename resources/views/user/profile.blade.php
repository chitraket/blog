            ...
            <div class="row justify-content-center d-flex">
                                    ...
                                    <p class="footer">
                                        @if (Auth::guest())
                                            <a href="javascript:void(0);"  onclick="toastr.info('To add favorite list. You need to login first.','Info',{
                                                closeButton: true,
                                                progressBar: true,
                                            })" class="text-dark pl-1"><i class="fa fa-heart-o " aria-hidden="true"></i>  {{ $post->favorite_post->count() }} {{__('profile.like')}}</a>
                                        @else
                                        <a href="javascript:void(0);" id="addfavourites{{$post->id}}" onClick="addToFavourites({{$post->id}}, {{ $post->admin->id }}).submit();" name="addToFavourites" class="text-dark pl-1">
                                            ...
                                            <span id="price{{$post->id}}">{{ $post->favorite_post->count() }}</span> {{__('profile.like')}}</a>
                                        @endif
                ...
                <div class="col-lg-4 sidebar-area">
                    <div class="single_widget about_widget">
                        ...
                        <a href="{{ route('profile',['locale'=>app()->getLocale(),'username'=>$author->username])}}"> <h2>{{ $author->name }}</h2></a>
                        <p>
                            {{$author->about}}
                        </p>
